...
bool BSplineCurve::run(const Points2d& points, Points3d& path)
{
  ...
  else
  {
    ...
    {
      auto prev_interp_vec = rpp_pkg::vec2d::Vec2d(waypoints[i - 1].x(), waypoints[i - 1].y());
      auto curr_interp_vec = rpp_pkg::vec2d::Vec2d(waypoints[i].x(), waypoints[i].y());
      auto next_interp_vec = rpp_pkg::vec2d::Vec2d(waypoints[i + 1].x(), waypoints[i + 1].y());
      auto tangent_dir = rpp_pkg::math::tangentDir(prev_interp_vec, curr_interp_vec, next_interp_vec, false);
