...
void RPPController::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros)
{
  if (!initialized_)
  {
    ...
    nh.param("min_lookahead_dist", min_lookahead_dist_, 0.6); // 最小前瞻距离（单位：米）：无论速度多慢，前瞻距离不小于该值
    ...
    // constriants -约束参数
    nh.param("regulated_min_radius", regulated_min_radius_, 2.0);   // 最小转弯半径0.9
    ...
    nh.param("approach_dist", approach_dist_, 1.2);              // 目标接近距离（单位：米）：小于该距离时开始减速靠近目标
...
bool RPPController::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
  ...
  // 计算前瞻点（L） - 固定为0.5米
  // const double L = 0.8;  // 固定前瞻距离为0.5米
...
{
  double dphi = atan2(lookahead_pt.point.y - robot_pose_global.pose.position.y,
               lookahead_pt.point.x - robot_pose_global.pose.position.x) -
         tf2::getYaw(robot_pose_global.pose.orientation);
  if (std::isnan(dphi)) {
    ROS_ERROR("dphi calculation resulted in NaN!");
    return 0.0;
  }
  return dphi;
}
...
double RPPController::_applyCurvatureConstraint(const double raw_linear_vel, const double curvature)
{
  ...
  if (std::isnan(radius)) {
    ROS_ERROR("Radius calculation resulted in NaN!");
