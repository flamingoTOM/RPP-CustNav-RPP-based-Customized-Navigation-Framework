...
Controller::Controller()
  : factor_(0.5), base_frame_("base_footprint"), map_frame_("map"), odom_frame_("odom"), costmap_ros_(nullptr)
{
...
{
  if (std::isnan(w_d) || std::isnan(base_odometry.twist.twist.angular.z)) {
    ROS_ERROR("Input contains NaN!");
    return 0.0;
  }
...
// /**
//  * @brief 线速度正则化（修改后）
//  * @param current_vel 从位姿计算的当前线速度
//  * @param v_d         期望线速度
//  * @return 正则化后的线速度指令
//  */
// double Controller::linearRegularization(double current_vel, double v_d)
// {
//   // 原逻辑：从里程计获取速度（删除）
//   // double v = std::hypot(base_odometry.twist.twist.linear.x, base_odometry.twist.twist.linear.y);
  
//   // 新逻辑：直接使用传入的当前速度（从位姿计算）
//   double v = current_vel;
...
// /**
//  * @brief 角速度正则化（修改后）
//  * @param current_ang_vel 从位姿计算的当前角速度
//  * @param w_d             期望角速度
//  * @return 正则化后的角速度指令
//  */
// double Controller::angularRegularization(double current_ang_vel, double w_d)
// {
//   // 检查输入有效性（原逻辑不变）
//   if (std::isnan(w_d) || std::isnan(current_ang_vel)) {
//     ROS_ERROR("Input contains NaN!");
//     return 0.0;
//   }
...
//   // 新逻辑：直接使用传入的当前角速度（从位姿计算）
//   double w = current_ang_vel;
                               ...
                               const geometry_msgs::PoseStamped& in_pose, geometry_msgs::PoseStamped& out_pose) const
{
    ...
    // 输出输入位姿的信息
    ROS_DEBUG("Input pose: x = %f, y = %f, z = %f", in_pose.pose.position.x, in_pose.pose.position.y, in_pose.pose.position.z);
    ROS_DEBUG("Input orientation: x = %f, y = %f, z = %f, w = %f", in_pose.pose.orientation.x, in_pose.pose.orientation.y, in_pose.pose.orientation.z, in_pose.pose.orientation.w);

    // 检查输入的位姿是否包含 NaN 值
    if (std::isnan(in_pose.pose.position.x) || std::isnan(in_pose.pose.position.y) || std::isnan(in_pose.pose.position.z) ||
        std::isnan(in_pose.pose.orientation.x) || std::isnan(in_pose.pose.orientation.y) || std::isnan(in_pose.pose.orientation.z) || std::isnan(in_pose.pose.orientation.w)) {
        ROS_ERROR("Input pose contains NaN values!");
        return;
    }

    try {
        tf->transform(in_pose, out_pose, out_frame, ros::Duration(1)); // 设置超时时间为 0.1 秒
